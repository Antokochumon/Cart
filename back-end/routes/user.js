...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'public/product-images'); // Save images in the 'public/product-images' folder
    },
...
router.post('/add-product', upload.fields([
    ...
    try {
        ...
        const product = {
            ...
            Price,
            Description,
            Image: `/product-images/${mainImage}`, // Main image path
            additionalImages: additionalImages.map(image => `/product-images/${image}`), // Additional image paths
        };

        await db.get().collection('product').insertOne(product);
        res.redirect('/admin/view-products'); // Redirect to the product list page
    } catch (err) {
